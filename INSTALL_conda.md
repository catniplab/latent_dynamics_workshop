...
[README](README.md). This conda path is kept for environments where conda is
preferred (e.g. institutional setups or when installing PyTorch through
conda-forge).
...
1. Clone with submodules (see the cloning step in the [README](README.md)).
...
With the environment activated, run the workshop commands directly (no `uv run`
prefix), e.g. generate the Van der Pol data and launch Jupyter:
```bash
python code_pack/generate_vdp_data.py
jupyter lab
```

For contributor tooling (nbstripout, jupytext), see the [README](README.md) -
install those tools into the conda environment with
