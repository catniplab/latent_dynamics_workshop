...
# BEGIN SOLUTION
b = -2.5
C = 3.0 * rng.standard_normal((nNeuron, dLatent))
C[:, 0] *= 0.5  # make the first latent dim less important
# END SOLUTION
...
#
# This optional comparison changes only the inverse-link that turns state into
# rate. The rest of the workshop uses the exponential link,
# $$ \lambda(t) = \exp(z(t)\, C^\top + b), $$
# but softplus is another valid nonnegative choice. It is near-linear for large
# drive and only bends exponentially near threshold,
...
# $z(t)$ (prescribed or dynamical), map it through a nonnegative rate link with
