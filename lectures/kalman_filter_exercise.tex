...
\begin{enumerate}
  ...
  Set $J'(x)=0$:
  \[
  \frac{2(x-\mu_b)}{P_b}-\frac{2(y-x)}{R}=0
  \;\Rightarrow\;
  R(x-\mu_b)=P_b(y-x)
  \;\Rightarrow\;
  x=\frac{R\mu_b+P_b y}{P_b+R}.
  \]
  Rewriting with $K=\dfrac{P_b}{P_b+R}$,
  \[
  x=\mu_b+\frac{P_b}{P_b+R}(y-\mu_b)=\mu_b+K(y-\mu_b).
  \]
  \end{solution}
  ...
  \begin{solution}
  The curvature of $J$ is $J''(x)=\dfrac{2}{P_b}+\dfrac{2}{R}$, and for a
...
\begin{solution}
Step 1: $\mu_1^m=a\mu_0=0$, $P_1^m=a^2P_0+Q=2+2=4$,
$K_1=\dfrac{4}{4+4}=0.5$, $\mu_1=0+0.5(2-0)=1$, $P_1=(1-0.5)\cdot4=2$.
The variance reaches its steady state ($P^m=4$, $K=0.5$, $P=2$) at once, so
...
\begin{enumerate}
  ...
  prediction $\mu_3^m=1.5$ toward the observation, $1.5+0.5(5-1.5)=3.25$. The
  ...
  \begin{solution}
  Smaller. $K_t=\dfrac{P_t^m}{P_t^m+R}$ shrinks as $R$ grows, so the update
...
\begin{enumerate}
  ...
  \begin{solution}
  $\boldsymbol{\mu}_1^m=A\boldsymbol{\mu}_0=\begin{bmatrix}0\\0\end{bmatrix}$,
  $\;P_1^m=AP_0A^\top+Q=2I+I=\begin{bmatrix}3&0\\0&3\end{bmatrix}$.
  \end{solution}
